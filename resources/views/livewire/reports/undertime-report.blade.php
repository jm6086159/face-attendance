<div class="p-6 space-y-6 text-gray-900">
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        <div>
            <h1 class="text-2xl font-semibold text-white">Undertime Report</h1>
            <p class="text-sm text-white/80">Monitor employees with undertime, identify patterns, and generate reports for accountability.</p>
        </div>
    ...
    <div class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            <div>
                <label class="text-xs text-gray-700">Minimum Undertime (minutes)</label>
                <input type="number" wire:model.live="minimumUndertime" min="1" class="w-full text-gray-900 rounded-lg border-gray-200 focus:border-[#0f8b48] focus:ring-[#0f8b48] bg-white">
            </div>
            <div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
            <div class="text-xs text-gray-700">Total Undertime Records</div>
            <div class="text-2xl font-semibold text-gray-900">{{ number_format($summary['total_records']) }}</div>
        </div>
        <div class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
            <div class="text-xs text-gray-700">Employees Affected</div>
            <div class="text-2xl font-semibold text-gray-900">{{ number_format($summary['unique_employees']) }}</div>
        </div>
        <div class="rounded-xl border border-red-200 bg-red-50 p-4 shadow-sm">
            <div class="text-xs text-red-700">Total Undertime</div>
            <div class="text-2xl font-semibold text-red-900">{{ $summary['total_undertime_hours'] ?? '00h 00m' }}</div>
        </div>
        <div class="rounded-xl border border-amber-200 bg-amber-50 p-4 shadow-sm">
            <div class="text-xs text-amber-700">Avg. Undertime/Record</div>
            <div class="text-2xl font-semibold text-amber-900">{{ $summary['avg_undertime_hours'] ?? '00h 00m' }}</div>
        </div>
    </div>
            ...
            <table class="min-w-full text-sm">
                <thead class="bg-gray-50 text-gray-700">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left font-medium">Undertime</th>
                    ...
                    @forelse($records as $record)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3">
                                @php
                                    $statusColors = [
                                        'Undertime' => 'bg-amber-100 text-amber-800',
                        ...
                        <tr>
                            <td colspan="10" class="px-4 py-8 text-center text-gray-500">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="text-lg font-medium">No undertime records found</p>
